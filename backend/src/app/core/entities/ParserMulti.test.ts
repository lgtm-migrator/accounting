...
function fakerLineMatcher(identifier: RegExp): ParserMulti.LineInfo {
	return {
		...
		accountTo: faker.random.number({ min: Consts.ACCOUNT_NUMBER_START, max: Consts.ACCOUNT_NUMBER_END }),
...
describe('Parser Multi #cold #entity', () => {
	...
	it('Matcher with year, month, and day', () => {
		parser.matcher = /(?<year>\d{2})(?<month>\d{2})(?<day>\d{2}); (?<name>.*?); (?<amount>-?\d*?\s?\d*)(; (?<currency>\w{3});|;)/g
	...
	it('Validate matcher date', () => {
		...
		// Missing day
		parser.matcher = parser.matcher = /(?<year>\d{2})(?<month>\d{2})(?<days>\d{2}); (?<name>.*?); (?<amount>-?\d*?\s?\d*)(; (?<currency>\w{3});|;)/g
		expect(parser.validate()).toStrictEqual([errorDay])

		// Missing month
		parser.matcher = parser.matcher = /(?<year>\d{2})(?<months>\d{2})(?<day>\d{2}); (?<name>.*?); (?<amount>-?\d*?\s?\d*)(; (?<currency>\w{3});|;)/g
		expect(parser.validate()).toStrictEqual([errorMonth])

		// Missing year
		parser.matcher = parser.matcher = /(?<years>\d{2})(?<month>\d{2})(?<day>\d{2}); (?<name>.*?); (?<amount>-?\d*?\s?\d*)(; (?<currency>\w{3});|;)/g
