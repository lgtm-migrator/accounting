...
export class MongoDbGateway implements DbGateway {
  ...
  constructor() {
    this.connectedPromise = MongoClient.connect(config.mongoDb.url())
      .then((mongoClient) => {
  ...
  private async collection(collection: Collections): Promise<Collection> {
    return this.waitUntilConnected().then(() => {
      if (!this.db) {
        throw new MongoRuntimeError('No DB connection found')
      }
      return this.db.collection(collection)
    })
  }

  private async save(entity: Entity, collection: Collections): Promise<Record<string,unknown>> {
  ...
  async saveVerification(verification: Verification): Promise<Verification> {
    return this.save(verification, Collections.Verification).then(
      (dbObject) => {
  ...
  async getExistingVerification(
    verification: Verification.Comparable
    ...
    return this.collection(Collections.User)
      .then((collection) => {
        return collection.findOne(
          ...
          { projection: { localCurrencyCode: 1 } }
        )
      })
      .then((foundObject) => {
        if (foundObject && foundObject.localCurrencyCode) {
          ...
          throw OutputError.create(
            ...
            foundObject.localCurrencyCode
          )
        } else {
          throw OutputError.create(
            OutputError.Types.userNotFound,
            String(userId)
          )
        }
      })
