...
describe('MongoDBGateway testing connection to the DB #db', () => {
  ...
  it('saveVerification() - Invalid', async () => {
    ...
    await expect(promise).rejects.toStrictEqual(
      OutputError.create(OutputError.Types.nameTooShort, '12')
    )
  })
  ...
  it('getVerification() searching for id that does not exist', async () => {
    ...
    const validError = OutputError.create(
      OutputError.Types.verificationNotFound,
      otherId
    )
  ...
  it('getVerification() verification id exists, but wrong userId', async () => {
    ...
    const promise = gateway.getVerification(
  ...
  it('getVerifications()', async () => {
    ...
    await expect(
      gateway.getVerifications(USER_ID, fiscalId1)
    ).resolves.toStrictEqual([userVer1])

    // Two verifications from the second fiscal id
    const promise = gateway.getVerifications(USER_ID, fiscalId2)
    await expect(promise).resolves.toContainEqual(
      expect.objectContaining(userVer2)
    )
    await expect(promise).resolves.toContainEqual(
      expect.objectContaining(userVer2)
    )
    await expect(promise).resolves.not.toContainEqual(
      expect.objectContaining(userVer1)
    )
    await expect(promise).resolves.not.toContainEqual(
      expect.objectContaining(otherVer)
    )

    // No verifications found
    await expect(
      gateway.getVerifications(
        new ObjectId().toHexString(),
        new ObjectId().toHexString()
      )
    ).resolves.toStrictEqual([])
    await expect(
      gateway.getVerifications(USER_ID, new ObjectId().toHexString())
    ).resolves.toStrictEqual([])
  })
  ...
  it('getExistingVerification()', async () => {
    ...
    await db
      .collection(Collections.Verification)
      .replaceOne({ _id: new ObjectId(validVerification.id) }, insertObject)
    promise = gateway.getExistingVerification(comparable)
    await expect(promise).resolves.toStrictEqual(validVerification)
  })
  ...
  it('getUnboundVerifications()', async () => {
    ...
    // Test data
    const promise = gateway.getUnboundVerifications(USER_ID)
    await expect(promise).resolves.toHaveLength(4)
    await expect(promise).resolves.toContainEqual(
      expect.objectContaining(unbound1)
    )
    await expect(promise).resolves.toContainEqual(
      expect.objectContaining(unbound2)
    )
    await expect(promise).resolves.toContainEqual(
      expect.objectContaining(unbound3)
    )
    await expect(promise).resolves.toContainEqual(
      expect.objectContaining(unbound4)
    )
  })
  ...
  it('getAccount()', async () => {
    ...
    const insertObject = MongoConverter.toDbObject(account)
    ...
    let validError = OutputError.create(
      OutputError.Types.accountNumberNotFound,
      String(1234)
    )
    await expect(promise).rejects.toStrictEqual(validError)
  ...
  it('saveParser() single', async () => {
    ...
    await promise
      .then((parser) => {
        return db
          .collection(Collections.Parser)
          .findOne({ _id: new ObjectId(parser.id) })
      })
      .then((object: any) => {
  ...
  it('getParsers()', async () => {
    ...
    await expect(promise).resolves.toContainEqual(
      expect.objectContaining(userParserSingle)
    )
    await expect(promise).resolves.toContainEqual(
      expect.objectContaining(userParserMulti)
    )

    // No parser
    await expect(
      gateway.getParsers(new ObjectId().toHexString())
    ).resolves.toStrictEqual([])
  })
  ...
  it('getLocalCurrency()', async () => {
    ...
    await db
      .collection(Collections.User)
      .insertOne(MongoConverter.toDbObject(user))
    ...
    await expect(promise).rejects.toStrictEqual(
      OutputError.create(OutputError.Types.userNotFound, otherId)
    )
  })
  ...
  it('getUser()', async () => {
    ...
    await expect(promise).rejects.toStrictEqual(
      OutputError.create(OutputError.Types.userNotFound)
    )
  })
  ...
  it('getFiscalYear()', async () => {
    ...
    // 2012
    await expect(
      gateway.getFiscalYear(USER_ID, '2012-01-01')
    ).resolves.toStrictEqual(fiscal2012)
    await expect(
      gateway.getFiscalYear(USER_ID, '2012-05-31')
    ).resolves.toStrictEqual(fiscal2012)
    await expect(
      gateway.getFiscalYear(USER_ID, '2012-12-31')
    ).resolves.toStrictEqual(fiscal2012)

    // 2013
    await expect(
      gateway.getFiscalYear(USER_ID, '2013-01-01')
    ).resolves.toStrictEqual(fiscal2013)
    await expect(
      gateway.getFiscalYear(USER_ID, '2013-05-31')
    ).resolves.toStrictEqual(fiscal2013)
    await expect(
      gateway.getFiscalYear(USER_ID, '2013-12-31')
    ).resolves.toStrictEqual(fiscal2013)

    // 2014
    await expect(
      gateway.getFiscalYear(USER_ID, '2014-01-01')
    ).resolves.toStrictEqual(fiscal2014)
    await expect(
      gateway.getFiscalYear(USER_ID, '2014-05-31')
    ).resolves.toStrictEqual(fiscal2014)
    await expect(
      gateway.getFiscalYear(USER_ID, '2014-12-31')
    ).resolves.toStrictEqual(fiscal2014)

    // 2011 - Not found
    await expect(
      gateway.getFiscalYear(USER_ID, '2011-01-01')
    ).rejects.toStrictEqual(
      OutputError.create(OutputError.Types.fiscalYearNotFound, '2011-01-01')
    )
    await expect(
      gateway.getFiscalYear(USER_ID, '2011-05-31')
    ).rejects.toStrictEqual(
      OutputError.create(OutputError.Types.fiscalYearNotFound, '2011-05-31')
    )
    await expect(
      gateway.getFiscalYear(USER_ID, '2011-12-31')
    ).rejects.toStrictEqual(
    ...
    // Invalid date format
    await expect(
      gateway.getFiscalYear(USER_ID, '2016-u1-12')
    ).rejects.toStrictEqual(
      OutputError.create(OutputError.Types.dateFormatInvalid, '2016-u1-12')
    )
    await expect(
      gateway.getFiscalYear(USER_ID, '2017-02-29')
    ).rejects.toStrictEqual(
  ...
  it('getFiscalYears()', async () => {
    ...
    const promise = gateway.getFiscalYears(USER_ID)
    await expect(promise).resolves.toContainEqual(
      expect.objectContaining(fiscal2012)
    )
    await expect(promise).resolves.toContainEqual(
      expect.objectContaining(fiscal2013)
    )
    await expect(promise).resolves.toContainEqual(
      expect.objectContaining(fiscal2014)
    )
    await expect(promise).resolves.not.toContainEqual(
      expect.objectContaining(fiscalOther)
    )

    // No verifications found
    await expect(
      gateway.getFiscalYears(new ObjectId().toHexString())
    ).resolves.toStrictEqual([])
  })
})
...
function fakerParserMulti(): Parser {
  return new ParserMulti({
    ...
    currencyCodeDefault: 'SEK',
    matcher:
      /(?<year>\d{2})(?<month>\d{2})(?<day>\d{2})\s+(?<name>.*?)\s{4}\s+(?<amount>-?\d*?\s?\d*)\s{4}/g,
    lineMatchers: [
      {
    ...
    startingBalances: [
      {
        accountNumber: faker.datatype.number({
          min: Consts.ACCOUNT_NUMBER_START,
          max: Consts.ACCOUNT_NUMBER_END,
        }),
