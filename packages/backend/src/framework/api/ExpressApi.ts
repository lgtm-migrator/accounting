...
export class ExpressApi {
  ...
  private bindVerification() {
    // Get all
    this.api.getAuthorized(
      '/api/verification/:fiscalYearId',
      (adapter, request, userId) => {
        const fiscalYearId: Id = ExpressSerializer.deserialize(
          request.params.fiscalYearId
        )
        return adapter.verification.getAll({
          userId: userId,
    ...
    this.api.postAuthorized(
      '/api/verification/add-custom',
      async (adapter, request, userId, files) => {
        const verifictationData = ExpressSerializer.deserialize(request.body)
        verifictationData.files = files
        const input: VerificationNewCustomTransactionInput = {
          userId: userId,
          verification: verifictationData,
        }
    ...
    // Add from parser
    this.api.postAuthorized(
      '/api/verification/add-from-files',
      async (adapter, request, userId, files) => {
        const input: VerificationNewFromParserInput = {
          userId: userId,
          files: files,
        }
