...
export class ExpressApiHelper {
  ...
  get(
    ...
    responseCode: ResponseCodes = ResponseCodes.OK
  ) {
  ...
  private simple(
    ...
    action(this.adapter, request)
      .then((apiOutput) => {
        response
          .status(responseCode)
          .send(ExpressSerializer.serialize(apiOutput));
      })
      .catch((reason) => {
  ...
  private authorized(
    ...
    if (typeof query.apiKey === "string") {
      ...
      response
        ...
        .send(OutputError.create(OutputError.Types.apiKeyMissing));
      return;
    }
    ...
    if (request.files) {
      ...
      if (request.files.files instanceof Array) {
        for (const file of request.files.files) {
          const uniqueFilename = ExpressApiHelper.generateUniqueFilename(
            file.name
          );
