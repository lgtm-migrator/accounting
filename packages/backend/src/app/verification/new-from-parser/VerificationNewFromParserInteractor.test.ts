...
class RepositoryTest
  extends VerificationRepositoryTest
...
describe('Verification new from parser #cold #use-case', () => {
    ...
    await output.then((output) => {
      ...
      for (const validTransaction of validTransactions) {
        expect(verification.transactions).toContainEqual(
          expect.objectContaining(validTransaction)
        )
      }
    })
  })
  ...
  it('Could not find parser', async () => {
    ...
    output = interactor.execute(input)
    await expect(output).rejects.toStrictEqual(
      OutputError.create(OutputError.Types.parserNotFound)
    )
  })
  ...
  it('File not found', async () => {
    ...
    output = interactor.execute(input)
    await expect(output).rejects.toStrictEqual(
      OutputError.create(OutputError.Types.internalError)
    )
  })
