...
export class Currency implements Currency.Option {
  ...
  constructor(options: Currency.Option) {
    ...
    if (typeof options.code === 'string') {
      const foundCode = Currency.Codes.fromString(options.code)
      if (foundCode) {
        this.code = foundCode
      } else {
    ...
    if (typeof options.localCode !== 'undefined') {
      if (typeof options.localCode === 'string') {
        const foundCode = Currency.Codes.fromString(options.localCode)
        if (foundCode) {
    ...
    // Convert from number to bigint
      ...
      this.amount = Currency.numberToBigInt(
        options.amount,
        this.code.precision
      )
    }
  ...
  private static numberToBigInt(value: number, precision: number): bigint {
    ...
    let out = BigInt(Math.round(value * 10 ** precision))

    // Calculate rest value so we round of correctly
    const rest: bigint =
      BigInt(Math.round(value * 10 ** (precision + 1))) % 10n
  ...
  split(fractions: number[]): Currency[] {
    ...
    for (let i = 0; i < amounts.length; ++i) {
      ...
      if (this.isLocalAmountSet()) {
        ...
        fractionLocalAmount = Currency.divideByPrecision(
          fractionLocalAmount,
      ...
      fractionAmount = Currency.divideByPrecision(
        fractionAmount,
        maxFractionPrecision
      )
   ...
   * @see isLargerThanEqualTo() to check which comparable amount is farthest away from 0
   */
  private getComparableResults(
    other: Currency,
    positive = false
  ): bigint[] {
    if (!this.isComparableTo(other)) {
      throw new InternalError(InternalError.Types.comparableError, [
        OutputError.Types.currenciesNotComparable,
      ])
    }
  ...
  isComparableTo(other: Currency): boolean {
    ...
    if (
      ...
      typeof this.localCode !== 'undefined'
    ) {
      return true
    }
  ...
  private calculateLocalAmount(): bigint {
    ...
    const precisionDiff = BigInt(
      this.localCode.precision - this.code.precision
    )
    ...
    const exchangeRatePrecision = Currency.calculatePrecision(
      this.exchangeRate!
    )

    // Make exchange rate as a bigint
    const exchangeRate = BigInt(
      Math.round(10 ** Number(exchangeRatePrecision) * this.exchangeRate!)
    )
  ...
  getLocalCurrency(): Currency {
    if (
      typeof this.localCode === 'undefined' ||
      typeof this.localAmount === 'undefined'
    ) {
      return this
    }
