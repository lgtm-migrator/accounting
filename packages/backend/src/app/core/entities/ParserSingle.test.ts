...
faker.seed(123);
const GOOGLE_INVOICE_FILE =
  "packages/backend/src/jest/test-data/Google-invoice.txt";
...
describe("ParserSingle #cold #entity", () => {
      ...
      const errors: OutputError.Info[] = [
        {
          ...
          data: String(data.matcher.date.find),
        },
        {
          type: OutputError.Types.parserPatternNotFound,
          data: String(data.matcher.currencyCode.find),
        },
        {
          type: OutputError.Types.parserPatternNotFound,
          data: String(data.matcher.amount.find),
        },
      ];
  ...
  it("Invalid currency code in parsed text", () => {
    ...
    try {
      ...
      for (const validError of validErrors) {
        expect(exception.errors).toContainEqual(
  ...
  it("Matcher missing replacement", () => {
    ...
    expect(parser.validate()).toStrictEqual([
      { type: OutputError.Types.parserMatcherReplacementMissing, data: "date" },
    ]);
  });
  ...
  it("Matcher missing replace", () => {
    ...
    expect(parser.validate()).toStrictEqual([
      { type: OutputError.Types.parserMatcherReplaceMissing, data: "date" },
    ]);
  });
  ...
  it("Matcher missing find", () => {
    ...
    expect(parser.validate()).toStrictEqual([
      { type: OutputError.Types.parserMatcherFindMissing, data: "date" },
    ]);
  });
