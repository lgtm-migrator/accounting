...
export class Verification extends UserEntity implements Verification.Option {
  ...
  validate(): OutputError.Info[] {
    ...
    if (
      this.internalName &&
      this.internalName.length < Consts.NAME_LENGTH_MIN
    ) {
      errors.push({
        type: OutputError.Types.internalNameTooShort,
        data: this.internalName,
      });
    }
    ...
    if (typeof this.fiscalYearId === "string") {
      if (this.fiscalYearId.length <= 0) {
        errors.push({
          type: OutputError.Types.verificationFiscalYearIdIsEmpty,
        });
      }
    }
  ...
  removeTransaction(accountNumber: number) {
    ...
    if (this.dateFiled) {
      ...
      throw OutputError.create(
        OutputError.Types.transactionNotFound,
        `${accountNumber}`
      );
    }
