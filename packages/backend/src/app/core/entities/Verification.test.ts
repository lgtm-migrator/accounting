...
describe('Verification test #cold #entity', () => {
  ...
  it('getFullName()', () => {
    ...
    expect(verification.getFullName()).toStrictEqual(
      '#0012 - 2012-01-01 - Name'
    )
  })
  ...
  it('getTransaction()', () => {
    ...
    // Valid
    expect(verification.getTransaction(2440)).toStrictEqual(
      verification.transactions[0]
    )
    expect(verification.getTransaction(4661)).toStrictEqual(
      verification.transactions[1]
    )
  ...
  it('removeTransaction() not filed yet', () => {
    ...
    try {
      ...
      expect(exception).toStrictEqual(
        OutputError.create(OutputError.Types.transactionNotFound, '2440')
      )
    }
  })
  ...
  it('removeTransaction() with filed verification', () => {
    ...
    expect(verification.dateModified).not.toStrictEqual(
      oldModifiedVerification
    )
    expect(verification.transactions[0].dateModified).not.toStrictEqual(
      oldModifiedTransaction
    )
    expect(verification.transactions[0].dateDeleted).not.toStrictEqual(
      undefined
    )
    ...
    const transaction = new Transaction(fakerTransaction())
  ...
  it('addTransaction()', () => {
    ...
    try {
      ...
      expect(exception).toStrictEqual(
        OutputError.create(
          OutputError.Types.transactionWithAccountNumberExists,
          '2222'
        )
      )
    }
  ...
  it('Too short name', () => {
    ...
    expect(verification.validate()).toStrictEqual([
      { type: OutputError.Types.nameTooShort, data: verification.name },
    ])
  })
    ...
    expect(verification.validate()).toStrictEqual([
      {
        ...
        data: verification.internalName,
      },
    ])
  })
  ...
  it('Invalid verification number (less than 1)', () => {
    ...
    expect(verification.validate()).toMatchObject([
      { type: OutputError.Types.verificationNumberInvalid },
    ])
  ...
  it('Verification number set, but missing date filed', () => {
    ...
    expect(verification.validate()).toStrictEqual([
      { type: OutputError.Types.verificationDateFiledMissing },
    ])
  })
  ...
  it('Date filed but missing verification number', () => {
    ...
    verification.dateFiled = verification.dateCreated
    expect(verification.validate()).toStrictEqual([
      { type: OutputError.Types.verificationNumberMissing },
    ])
  })

  it('Date filed before creation date', () => {
    verification.number = 1
    verification.dateFiled = faker.date
      .between('2010-01-01', '2014-12-31')
      .getTime()
    verification.dateCreated = faker.date
      .between('2015-01-01', '2019-12-31')
      .getTime()
    expect(verification.validate()).toMatchObject([
      { type: OutputError.Types.verificationDateFiledBeforeCreated },
    ])
  })
  ...
  it('Validate other ids', () => {
    const validError = [
      { type: OutputError.Types.verificationPaymentIdIsEmpty },
    ]
  ...
  it('Transaction sum is not zero', () => {
    ...
    expect(verification.validate()).toStrictEqual([
      { type: OutputError.Types.transactionSumIsNotZero, data: '1' },
    ])
  })
  ...
  it('Transaction local code mismatch', () => {
    ...
    expect(verification.validate()).toMatchObject([
      { type: OutputError.Types.transactionsCurrencyCodeLocalMismatch },
    ])
  })
  ...
  it('Verification from option test', () => {
    ...
    expect(verification.totalAmount).toStrictEqual(
      new Currency(option.totalAmount!)
    )
    ...
    expect(verification.requireConfirmation).toStrictEqual(
