...
function fakerLineMatcher(identifier: RegExp): ParserMulti.LineInfo {
  return {
    ...
    accountTo: faker.datatype.number({
      min: Consts.ACCOUNT_NUMBER_START,
      max: Consts.ACCOUNT_NUMBER_END,
    }),
...
describe("Parser Multi #cold #entity", () => {
  ...
  it("Replace name", () => {
    ...
    const text =
      "2020-01-01; some line name; 1;" + "2020-01-01; another line; 1;";
  ...
  it("Validate matcher name", () => {
    parser.matcher =
      /(?<date>\d{4}-\d{2}-\d{2}); (?<namee>.*?); (?<amount>-?\d*?\s?\d*)(; (?<currency>\w{3});|;)/g;
    ...
    parser.matcher =
      /(?<date>\d{4}-\d{2}-\d{2}); .*?; (?<amount>-?\d*?\s?\d*)(; (?<currency>\w{3});|;)/g;
    expect(parser.validate()).toStrictEqual([
      {
        type: OutputError.Types.parserMatcherGroupMissing,
        data: "name",
  ...
  it("Validate matcher amount", () => {
    parser.matcher =
      /(?<date>\d{4}-\d{2}-\d{2}); (?<name>.*?); (?<amountt>-?\d*?\s?\d*)(; (?<currency>\w{3});|;)/g;
    ...
    parser.matcher =
      /(?<date>\d{4}-\d{2}-\d{2}); (?<name>.*?); (-?\d*?\s?\d*)(; (?<currency>\w{3});|;)/g;
    expect(parser.validate()).toStrictEqual([
      {
        type: OutputError.Types.parserMatcherGroupMissing,
        data: "amount",
  ...
  it("Validate matcher date", () => {
    ...
    parser.matcher =
      /(?<daete>\d{4}-\d{2}-\d{2}); (?<name>.*?); (?<amount>-?\d*?\s?\d*)(; (?<currency>\w{3});|;)/g;
    for (const validError of validErrors) {
      expect(parser.validate()).toContainEqual(
        expect.objectContaining(validError)
      );
    }

    parser.matcher =
      /(\d{4}-\d{2}-\d{2}); (?<name>.*?); (?<amount>-?\d*?\s?\d*)(; (?<currency>\w{3});|;)/g;
    for (const validError of validErrors) {
      expect(parser.validate()).toContainEqual(
        expect.objectContaining(validError)
      );
  ...
  it("Test for invalid currency code", () => {
    ...
    try {
      ...
      expect(exception.errors).toStrictEqual([
        { type: OutputError.Types.currencyCodeInvalid, data: "XTT" },
      ]);
    }
  });
});
